document.addEventListener('DOMContentLoaded', () => {
    ...
    async function createCard(image) {
        const card = document.createElement('div');
        card.className = 'card';

        const img = document.createElement('img');
        img.dataset.src = image.primaryImageSmall || 'https://www.italfren.com.ar/images/catalogo/imagen-no-disponible.jpeg';
        ...
        title.textContent = await translateText(image.title || 'No Title', 'es');

        const culture = document.createElement('p');
        culture.textContent = await translateText(image.culture ? `Cultura: ${image.culture}` : 'Cultura: No disponible', 'es');

        const dynasty = document.createElement('p');
        dynasty.textContent = await translateText(image.dynasty ? `Dinastía: ${image.dynasty}` : 'Dinastía: No disponible', 'es');
    ...
    async function displayImages(images) {
        ...
        const cards = await Promise.all(images.map(createCard));
        hideLoadingSpinner();
